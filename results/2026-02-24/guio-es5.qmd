...
curl_download(
  ...
  destfile = 'z1.txt')
```#En el código anterior, falta la expresión URLencode y no hay ningún paréntesis después del "0".En el código anterior, falta la expresión URLencode y no hay ningún paréntesis después del "0". 
...
fields <- 'fields=tax_id,scientific_name,last_updated,base_count,description&,first_public'
limit  <- 'limit=100000'
URL <- paste0(portal, result, query, fields, limit)
